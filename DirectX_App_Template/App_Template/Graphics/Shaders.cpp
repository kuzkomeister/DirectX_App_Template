...
bool VertexShader::Initialize(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::wstring shaderpath, D3D11_INPUT_ELEMENT_DESC* layoutDesc, UINT numElements)
{
	...
	if (FAILED(hr))
	{
		std::wstring errorMsg = L"Ошибка при загрузке вершинного шейдера: ";
		errorMsg += shaderpath;
	...
	if (FAILED(hr))
	{
		std::wstring errorMsg = L"Ошибка при создании вершинного шейдера: ";
		errorMsg += shaderpath;
		ErrorLogger::Log(hr, errorMsg);
		return false;
	}
	...
	if (FAILED(hr))
	{
		ErrorLogger::Log(hr, "Ошибка при создании input layout.");
		return false;
	}
